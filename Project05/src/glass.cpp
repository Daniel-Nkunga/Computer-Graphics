...
ColorRGB shade_point(Point3D &hit, Vector3D &norm, ColorRGB &obj_color,
                     ...
                     Point3D &camera)
{
   ColorRGB light_color; light_color.set(255, 255, 255);
   Vector3D light_dir;   light_dir.set(1.0f, 2.0f, 1.0f); light_dir.normalize();
...
{
   Point3D  hit; Vector3D norm;
   if (cube.get_intersection(ray, hit, norm))
   {
      if (mode == "normal")
      {
         ColorRGB c;
         c.set(127 + norm.vx * 127,
               127 + norm.vy * 127,
               127 + norm.vz * 127);
         return c;
      }
      ColorRGB obj_color; obj_color.set(0, 100, 200);
      return shade_point(hit, norm, obj_color, 0.2f, 0.5f, 0.5f, 20.0f, camera);
   }
   // Black background
   ColorRGB black; black.set(0, 0, 0);
   return black;
}
...
void ray_trace()
{
   ...
   Point3D camera;
   camera.set(cam_dist * cos(theta_y) * sin(theta),
              cam_dist * sin(theta_y),
   ...
   Cube3D cube;
   { Point3D c; c.set(0.0f, 0.0f, 2.0f); cube.set(c, 1.0f); }

   // Glass slab: sits at z=0, normal +Z, thickness 0.3
   Glass3D glass;
   {
      Point3D  gc; gc.set(0.0f, 0.0f, 0.0f);
      Vector3D gn; gn.set(0.0f, 0.0f, 1.0f);
      ColorRGB gt; gt.set(200, 235, 255);   // faint blue tint
   ...
   for (int x = 0; x < XDIM; x++)
   {
      ...
      Ray3D ray; ray.set(camera, vp);

      // --- Glass intersection ---
      Point3D  glass_hit; Vector3D glass_norm;
      ...
      else
      {
         ...
         if (hit_cube && cube_t < glass_t)
         {
            ...
            {
               ColorRGB obj; obj.set(0, 100, 200);
         ...
         else
         {
            ...
            if (mode == "normal")
            {
               pixel.set(127 + glass_norm.vx*127,
                         127 + glass_norm.vy*127,
                         127 + glass_norm.vz*127);
            }
            else
            {
               // Refracted ray (sees cube behind glass)
               Point3D exit_pt; Ray3D refr_ray;
               bool refracted = glass.get_refracted_ray(ray, glass_hit, exit_pt, refr_ray);
               ...
               Ray3D refl_ray; refl_ray.set(glass_hit, rdir);
               ...
               ColorRGB white; white.set(255, 255, 255);
               ColorRGB spec = shade_point(glass_hit, glass_norm,
                                           white, 0.0f, 0.0f, 0.5f, 80.0f, camera);
               pixel.add(spec);
               pixel.clamp();
            }
         }
...
void keyboard(unsigned char key, int x, int y)
{
   switch (key)
   {
      ...
      case 'Y': theta_y += step;                          break;
      case '+': if (cam_dist > 2.0f)  cam_dist /= 1.1f;  break;
      case '-': if (cam_dist < 30.0f) cam_dist *= 1.1f;  break;
