//---------------------------------------
// Program: cone.cpp
// Purpose: Demonstrate ray tracing with a cone.
...
void ray_trace() {
  ...
  Cone3D cone;
  Point3D apex;
  apex.set(0, -1.5, 3); // apex near bottom-center of view
  Vector3D axis;
  axis.set(0, 1, 0);             // opens upward
  float half_angle = M_PI / 6.0; // 30 degrees
  float height = 3.0;
  cone.set(apex, axis, half_angle, height);
  ...
  for (int y = 0; y < YDIM; y++)
    for (int x = 0; x < XDIM; x++) {
      ...
      // Perform cone intersection
      Point3D p;
      Vector3D n;
      if (cone.get_intersection(ray, p, n)) {
...
int main(int argc, char *argv[]) {
  ...
  glutCreateWindow("Ray Trace - Cone");
  init();
